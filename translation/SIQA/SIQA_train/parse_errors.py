...
def fix_and_escape_json(input_string):
    ...
    try:
        ...
        fixed_string = re.sub(r'("context":.*?)(?=\s*?"question":)(?!,)', r'\1, ', processed_string)
        fixed_string = re.sub(r'("question":.*?)(?=\s*?"answerA":)(?!,)', r'\1, ', fixed_string)
        fixed_string = re.sub(r'("answerA":.*?)(?=\s*?"answerB":)(?!,)', r'\1, ', fixed_string)
        fixed_string = re.sub(r'("answerB":.*?)(?=\s*?"answerC":)(?!,)', r'\1, ', fixed_string)
...
def get_original_entry(index,file_path="./SIQA_train.jsonl"):
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def retry_translation_with_temperature(entry, index, temperature=0.0,output_dir="./api_responses"):
    ...
    answerA = entry.get('answerA', '')
    answerB = entry.get('answerB', '')
    answerC = entry.get('answerC', '')
...
5. you will be passed a dictionary with the keys "context", "question", "answerA", "answerB", and "answerC". Do not translate the keys, only translate their values, and update the dictionary with translated values.
...
def replace_affected_entry(index, fixed_entry,output_file_path="./SIQA_train_bengali_gpt4omini.jsonl"):
    try:
...
def detect_english_entries(file_path="./SIQA_train_bengali_gpt4omini.jsonl", log_file_path="./logs/translation.log"):
    ...
    with open(output_file, 'w', encoding='utf-8') as outfile, open(error_log_file, 'w', encoding='utf-8') as errorfile:
        for index in indexes:
            try:
                ...
                combined_original_length = len(original_entry.get('context', '')) + \
                                           len(original_entry.get('question', '')) + \
                                           len(original_entry.get('answerA', '')) + \
                                           len(original_entry.get('answerB', '')) + \
                                           len(original_entry.get('answerC', '')) 
