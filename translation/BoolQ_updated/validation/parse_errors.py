...
def fix_and_escape_json(input_string):
    ...
    fixed_string = re.sub(r'("question":.*?)(?=\s*"title":)', r'\1,', fixed_string)
    fixed_string = re.sub(r'("title":.*?)(?=\s*"answer":)', r'\1,', fixed_string)
    fixed_string = re.sub(r'("answer":.*?)(?=\s*"passage":)', r'\1,', fixed_string)
...
def get_original_entry(index,file_path="./boolq_validation.jsonl"):
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def retry_translation_with_temperature(entry, index, temperature=0.0,output_dir="./api_responses"):
    ...
    answer = entry.get('answer', None)  # Boolean value
    passage = entry.get('passage', '')

    # Serialize values to JSON strings
    question_json = json.dumps(question, ensure_ascii=False)
    title_json = json.dumps(title, ensure_ascii=False)
    answer_json = json.dumps(answer)  # Booleans are serialized as true/false
...
5. Do not translate the keys "question", "title", "answer", and "passage". Only translate their corresponding values. Do not change the value of the key "answer".
...
def replace_affected_entry(index, fixed_entry,output_file_path="./boolq_validation_gpt4omini.jsonl"):
    try:
...
def detect_english_entries(file_path="./boolq_validation_gpt4omini.jsonl", log_file_path="./logs/translation.log"):
        ...
        with open(file_path, "r", encoding="utf-8") as file, open(log_file_path, "a", encoding="utf-8") as log_file:
            for index, line in enumerate(file):
                try:
                    ...
                    combined_text = f"{entry.get('question', '')} {entry.get('title', '')} {entry.get('answer', '')}{entry.get('passage', '')}"
        ...
        for index in indexes:
            try:
                ...
                combined_original_length = len(original_entry.get('question', '')) + \
                                           len(original_entry.get('title', '')) + \
                                           len(original_entry.get('passage', '')) 
