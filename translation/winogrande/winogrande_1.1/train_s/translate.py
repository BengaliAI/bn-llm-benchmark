...
jsonl_file_path = "./train_s.jsonl"
json_output_file_path = "./winogrande_train_s_gpt4omini_translated.jsonl"
...
def transform_translation(translation, idx):
    ...
    translation = preprocess_string(translation)
    ...
    try:
        ...
        def escape_value(match):
            ...
            return f'{key}: "{escaped_value}"'
        translation_fixed = re.sub(r'("(?:\\.|[^"\\])*?")\s*:\s*"(.*?)(?<!\\)"(?=\s*[,\n}])',escape_value, translation_fixed, flags=re.DOTALL)
