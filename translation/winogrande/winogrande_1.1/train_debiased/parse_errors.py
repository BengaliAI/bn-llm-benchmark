...
def fix_and_escape_json(input_string):
    ...
    try:
        ...
        fixed_string = re.sub(r'("option2":.*?)(?=\s*?"answer":)(?!,)', r'\1, ', fixed_string)
...
def get_original_entry(index,file_path="./train_debiased.jsonl"):
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def replace_affected_entry(index, fixed_entry,output_file_path="./winogrande_train_debiased_gpt4omini_translated.jsonl"):
    try:
...
def detect_english_entries(file_path="./winogrande_train_debiased_gpt4omini_translated.jsonl", log_file_path="./logs/translation.log"):
        ...
        with open(file_path, "r", encoding="utf-8") as file, open(log_file_path, "a", encoding="utf-8") as log_file:
            for index, line in enumerate(file):
                try:
                    ...
                    combined_text = f"{entry.get('qID', '')} {entry.get('sentence', '')} {entry.get('option1', '')} {entry.get('option2', '')} {entry.get('option2', '')}"
