...
def fix_and_escape_json(input_string):
    ...
    try:
        ...
        fixed_string = re.sub(r'("goal":.*?)(?=\s*?"sol1":)(?!,)', r'\1, ', processed_string)
        fixed_string = re.sub(r'("sol1":.*?)(?=\s*?"sol2":)(?!,)', r'\1, ', fixed_string)
        fixed_string = re.sub(r'("sol2":.*?)(?=\s*?"label":)(?!,)', r'\1, ', fixed_string)
...
def get_original_entry(index,file_path="./piqa_train.jsonl"):
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def replace_affected_entry(index, fixed_entry,output_file_path="./piqa_train_gpt4omini.jsonl"):
    try:
...
def detect_english_entries(file_path="./piqa_train_gpt4omini.jsonl", log_file_path="./logs/translation.log"):
        ...
        with open(file_path, "r", encoding="utf-8") as file, open(log_file_path, "a", encoding="utf-8") as log_file:
            for index, line in enumerate(file):
                try:
                    ...
                    # Concatenate the values of 'goal', 'sol1', and 'sol2' for language detection
                    combined_text = f"{entry.get('goal', '')} {entry.get('sol1', '')} {entry.get('sol2', '')}"
                    # Detect the language
    ...
    with open(output_file, 'w', encoding='utf-8') as outfile, open(error_log_file, 'w', encoding='utf-8') as errorfile:
        for index in indexes:
            try:
                ...
                combined_original_length = len(original_entry.get('goal', '')) + \
                                           len(original_entry.get('sol1', '')) + \
                                           len(original_entry.get('sol2', ''))
