...
def fix_and_escape_json(input_string):
    ...
    try:
        ...
        fixed_string = re.sub(r'("question":*?)(?=\s*?"question_concept":)(?!,)', r'\1, ', fixed_string)
        fixed_string = re.sub(r'("question_concept":.*?)(?=\s*?"choices":)(?!,)', r'\1, ', fixed_string)
...
def get_original_entry(index,file_path="./commonsenseqa_train.jsonl"):
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def retry_translation_with_temperature(entry, index, temperature=0.0,output_dir="./api_responses"):
    ...
    label_mapping = {"A": "ক", "B": "খ", "C": "গ", "D": "ঘ","E": "ঙ"}
...
def replace_affected_entry(index, fixed_entry,output_file_path="./commonsenseqa_train_4omini.jsonl"):
    try:
...
def detect_english_entries(file_path="./commonsenseqa_train_4omini.jsonl", log_file_path="./logs/translation.log"):
        ...
        for index in indexes:
            try:
                ...
                combined_original_length = len(original_entry.get('question', '')) + \
                                           len(original_entry.get('question_concept', '')) + \
                ...
                if translation:
                    translation = transform_translation(translation)
                    translation = fix_and_escape_json(translation)
                    translation_dict = json.loads(translation)
                    combined_translation_length = len(translation_dict.get('question', '')) + \
                              len(translation_dict.get('question_concept', '')) + \
