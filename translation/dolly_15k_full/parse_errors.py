...
def fix_and_escape_json(input_string):
    ...
    input_string = re.sub(r'\\?"(\d)"\\?', r'\1', input_string)
    input_string = re.sub(r'"({\\[^}]+})"', r'\1', input_string)
    input_string = re.sub(r'"(\d+(\.\d+)?)"', r'\1', input_string)
    input_string = re.sub(r'\\"(\d+(\.\d+)?)\\"', r'\1', input_string)
    input_string = re.sub(r'\\"([A-Za-z])\\"', r'\1', input_string)
    input_string = re.sub(r'"([A-Za-z])"', r'\1', input_string)
    input_string = re.sub(r'\(\\?"(\d[\d\s]*)\\?"\)', r'(\1)', input_string)
    input_string = re.sub(r'"\$([^$]+)\$"', r'$\1$', input_string)
    input_string = re.sub(r'\\"\$([^$]+)\$\\"', r'$\1$', input_string)
    input_string = re.sub(r'\\"(\$[^$]+\$)\\"', r'\1', input_string)
    input_string = re.sub(r'"(\$[^$]+\$)"', r'\1', input_string)
    ...
    fixed_string = re.sub(r'("instruction":.*?)(?=\s*"context":)', r'\1,', fixed_string)
    fixed_string = re.sub(r'("context":.*?)(?=\s*"response":)', r'\1,', fixed_string)
    fixed_string = re.sub(r'("response":.*?)(?=\s*"category":)', r'\1,', fixed_string)
...
def get_original_entry(index,file_path="./databricks-dolly-15k.jsonl"):
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def retry_translation_with_temperature(entry, index, temperature=0.0,output_dir="./api_responses"):
    instruction = json.dumps(entry.get("instruction", ""), ensure_ascii=False)
    context = json.dumps(entry.get("context", ""), ensure_ascii=False)
    response = json.dumps(entry.get("response", ""), ensure_ascii=False)
...
5. You will be passed a dictionary with the keys "instruction", "context" , "response" and "category". Do not translate the keys, only translate their values, and update the dictionary with translated values. Do not translate the value of the key "category".
...
def replace_affected_entry(index, fixed_entry,output_file_path="./databricks-dolly-15k_translated.jsonl"):
    try:
...
def detect_english_entries(file_path="./databricks-dolly-15k_translated.jsonl", log_file_path="./logs/translation.log"):
        ...
        with open(file_path, "r", encoding="utf-8") as file, open(log_file_path, "a", encoding="utf-8") as log_file:
            for index, line in enumerate(file):
                try:
                    ...
                    combined_text = f"{entry.get('instruction', '')} {entry.get('context', '')} {entry.get('response', '')} {entry.get('category', '')}"
    ...
    with open(output_file, 'w', encoding='utf-8') as outfile, open(error_log_file, 'w', encoding='utf-8') as errorfile:
        for index in indexes:
            try:
                ...
                combined_original_length = len(original_entry.get('instruction', '')) + \
                                           len(original_entry.get('context', '')) + \
                                           len(original_entry.get('response', '')) + \
                ...
                if translation:
                    ...
                    if combined_translation_length > (3 * combined_original_length):
                        ...
                        translation = retry_translation_with_temperature(original_entry, index,temperature=0.0)
                    ...
                    retry_translation = retry_translation_with_temperature(
                        original_entry, index,temperature=0.0
                    )
