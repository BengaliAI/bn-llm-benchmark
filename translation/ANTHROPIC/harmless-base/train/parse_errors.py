...
def fix_and_escape_json(input_string):
    ...
    fixed_string = re.sub(r'("chosen":.*?)(?=\s*"rejected":)', r'\1,', fixed_string)
...
def get_original_entry(index,file_path="./train.jsonl"):
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def replace_affected_entry(index, fixed_entry,output_file_path="./harmless_base_train_gpt4omini.jsonl"):
    try:
...
def detect_english_entries(file_path="./harmless_base_train_gpt4omini.jsonl", log_file_path="./logs/translation.log"):
        ...
        with open(file_path, "r", encoding="utf-8") as file, open(log_file_path, "a", encoding="utf-8") as log_file:
            for index, line in enumerate(file):
                try:
                    ...
                    combined_text = f"{entry.get('chosen', '')} {entry.get('rejected', '')}"
    ...
    with open(output_file, 'w', encoding='utf-8') as outfile, open(error_log_file, 'w', encoding='utf-8') as errorfile:
        for index in indexes:
            try:
                ...
                combined_original_length = len(original_entry.get('chosen', '')) + \
                           len(original_entry.get('rejected', '')) 
