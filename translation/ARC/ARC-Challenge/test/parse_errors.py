...
def get_original_entry(index,file_path="./arc_challenge_test.jsonl"):
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def retry_translation_with_temperature(entry, index, temperature=0.0,output_dir="./api_responses"):
    label_mapping = {
    "A": "ক", "B": "খ", "C": "গ", "D": "ঘ",  # Mapping for A, B, C, D
...
def replace_affected_entry(index, fixed_entry,output_file_path="./arc_challenge_test_gpt4omini.jsonl"):
    try:
...
def detect_english_entries(file_path="./arc_challenge_test_gpt4omini.jsonl", log_file_path="./logs/translation.log"):
