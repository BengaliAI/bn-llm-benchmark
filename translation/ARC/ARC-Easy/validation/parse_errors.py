...
def fix_and_escape_json(input_string):
    ...
    try:
        ...
        fixed_string = re.sub(r'("id":.*?)(?=\s*?"question":)(?!,)', r'\1, ', input_string)
        fixed_string = re.sub(r'("question":.*?)(?=\s*?"choices":)(?!,)', r'\1, ', fixed_string)
        fixed_string = re.sub(r'("choices":.*?)(?=\s*?"answerKey":)(?!,)', r'\1, ', fixed_string)
        fixed_string = re.sub(r',\s*,', ',', fixed_string)
...
def get_original_entry(index,file_path="./arc_easy_validation.jsonl"):
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def retry_translation_with_temperature(entry, index, temperature=0.0,output_dir="./api_responses"):
    label_mapping = {
    "A": "ক", "B": "খ", "C": "গ", "D": "ঘ","E":"ঙ",  # Mapping for A, B, C, D
    "1": "ক", "2": "খ", "3": "গ", "4": "ঘ"   # Mapping for 1, 2, 3, 4
    }
    qID = entry.get('id', '')
    ...
    # Map the labels using label_mapping
    original_labels = choices.get('label', [])
    mapped_labels = [label_mapping.get(label, label) for label in original_labels]
    choices_text = choices.get('text', [])

    # Prepare the choices as a dictionary
    choices_dict = dict(zip(mapped_labels, choices_text))

    # Serialize values to JSON strings
    qID_json = json.dumps(qID)
    question_json = json.dumps(question, ensure_ascii=False)
    choices_json = json.dumps(choices_dict, ensure_ascii=False)
    answerKey_mapped = label_mapping[answerKey]
    answerKey_json = json.dumps(answerKey_mapped)
...
5. you will be passed a dictionary with the keys "id", "question", "choices", and "answerkey". Do not translate the keys, only translate their values, and update the dictionary with translated values. Do not translate the values of the keys "id" and "answerkey".
...
def replace_affected_entry(index, fixed_entry,output_file_path="./arc_easy_validation_gpt4omini.jsonl"):
    try:
...
def detect_english_entries(file_path="./arc_easy_validation_gpt4omini.jsonl", log_file_path="./logs/translation.log"):
        ...
        with open(file_path, "r", encoding="utf-8") as file, open(log_file_path, "a", encoding="utf-8") as log_file:
            for index, line in enumerate(file):
                try:
                    ...
                    combined_text = f"{entry.get('question', '')} " \
                        f"ক: {entry['choices'].get('ক', '')} " \
                        f"খ: {entry['choices'].get('খ', '')} " \
                        f"গ: {entry['choices'].get('গ', '')} " \
                        f"ঘ: {entry['choices'].get('ঘ', '')} " \
                        f"ঘ: {entry['choices'].get('ঙ', '')} " \
        ...
        for index in indexes:
            try:
                ...
                combined_original_length = len(original_entry.get('question', '')) + \
                           sum(len(key) + len(value) for key, value in original_entry.get('choices', {}).items()) + \
                           len(original_entry.get('answerKey', ''))
                ...
                if translation:
                    ...
                    combined_translation_length = len(translation_dict.get('question', '')) + \
                              sum(len(key) + len(value) for key, value in translation_dict.get('choices', {}).items()) + \
                              sum(len(label) for label in translation_dict.get('answerKey', []))
