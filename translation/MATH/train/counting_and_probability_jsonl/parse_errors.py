...
def fix_and_escape_json(input_string):
    ...
    fixed_string = re.sub(r'("problem":.*?)(?=\s*"level":)', r'\1,', fixed_string)
    fixed_string = re.sub(r'("level":.*?)(?=\s*"type":)', r'\1,', fixed_string)
    fixed_string = re.sub(r'("type":.*?)(?=\s*"solution":)', r'\1,', fixed_string)
...
def get_original_entry(index,file_path="./counting_and_probability.jsonl"):
    try:
        with open(file_path, "r", encoding="utf-8") as file:
...
def retry_translation_with_temperature(entry, index, temperature=0.0,output_dir="./api_responses"):
    # Extract relevant fields from the entry based on the new dataset structure
    problem = entry.get('problem', '')
    level = entry.get('level', '')
    type_s = entry.get('type', '')
    solution = entry.get('solution', '')

    # Create a JSON structure with all fields
    entry_json = {
        "problem": problem,
        "level": level,
        "type" : type_s,
...
def replace_affected_entry(index, fixed_entry,output_file_path="./counting_and_probability_translated.jsonl"):
    try:
...
def detect_english_entries(file_path="./counting_and_probability_translated.jsonl", log_file_path="./logs/translation.log"):
        ...
        with open(file_path, "r", encoding="utf-8") as file, open(log_file_path, "a", encoding="utf-8") as log_file:
            for index, line in enumerate(file):
                try:
                    ...
                    combined_text = f"{entry.get('problem', '')} {entry.get('level', '')} {entry.get('type', '')} {entry.get('solution', '')}"
    ...
    with open(output_file, 'w', encoding='utf-8') as outfile, open(error_log_file, 'w', encoding='utf-8') as errorfile:
        for index in indexes:
            try:
                ...
                combined_original_length = len(original_entry.get('problem', '')) + \
                                           len(original_entry.get('level', '')) + \
                                           len(original_entry.get('type', '')) + \
                                           len(original_entry.get('solution', ''))
